...
/**
 * <p>
 * Logger的代码还是比较清晰的。首先Logger主要是实现了三个接口：
 * <ul>
 *     <li>org.slf4j.Logger</li>
 *     <li>org.slf4j.spi.LocationAwareLogger</li>
 *     <li>ch.qos.logback.core.spi.AppenderAttachable</li>
 * </ul>
 * 因此，也可以从这三个接口了解到Logger代码的主要结构。
 * </p>
 * <p>
 *     首先是有关org.slf4j.Logger接口的。在本类中，有大量的方法都是为了实现这个接口而写的。
 *     但这些方法最终都会被委托到几个集中的方法进行执行：
 *     <ul>
 *         <li>filterAndLog_0_Or3Plus(String localFQCN, Marker marker, Level level, String msg, Object[] params, Throwable t)</li>
 *         <li>filterAndLog_1(String localFQCN, Marker marker, Level level, String msg, Object param1, Throwable t)</li>
 *         <li>filterAndLog_2(String localFQCN, Marker marker, Level level, String msg, Object param1, Object param2, Throwable t)</li>
 *     </ul>
 *     而这三个方法的主要动作就是判断当前的Logger等级能否进行输出。如果可以输出的话，则会调用另一个方法：buildLoggingEventAndAppend
 * </p>
 * <p>
 *     然后buildLoggingEventAndAppend方法会调用callAppenders方法，这个方法会迭代出发自身的以及父级Logger的Appender去执行输出，
 *     除非是被Logger设置了additive字段为false。
 * </p>
 * <p>
 *     ch.qos.logback.core.spi.AppenderAttachable接口主要是使得Logger类具有添加Appenders的能力。
 *     但实际上所有针对这个接口的方法的调用最终都是被委托到ch.qos.logback.core.spi.AppenderAttachableImpl这个类的对象上。
 * </p>
 * <p>
 *     剩下的一些方法主要是针对Logger中的一些字段的setter和getter方法，这些setter方法中会执行一些字段被设置时的逻辑。
 * </p>
 */
public final class Logger implements org.slf4j.Logger, LocationAwareLogger,
   ...
   * the 'aai' variable is set is within the addAppender method. This method is
